...
function formatMonthTitle(dateString: string) {
  if (!dateString) return "Monthly Schedule";
...
function convertOldTableToGroups(schedule: Schedule): BuilderGroup[] {
  const oldColumns = schedule.table_json?.columns ?? ["Pelayan"];
  const oldRows = schedule.table_json?.rows ?? [];

  const columns = oldColumns.slice(1).map((column) => makeColumn(column, null));
  const safeColumns = columns.length > 0 ? columns : [makeColumn("Pelayan")];
  ...
  return [
    {
      ...
      rows: oldRows.map((row) => ({
        ...
        label: row.label,
        serving_role_id: null,
        cells: safeColumns.map((_, index) => ({
          display_name: row.cells[index] ?? "",
          profile_id: null,
          email: null,
        })),
      })),
...
function normalizeJoinedObject<T>(value: T | T[] | null | undefined): T | null {
...
export default function EditSchedulePage() {
    ...
    const load = async () => {
      ...
      if (schedule.division_id) {
        const { data: divisionMember } = await supabase
          ...
          .select("role")
          .eq("church_id", access.churchId)
          .eq("division_id", schedule.division_id)
      ...
      if (!canEdit) {
        setRedirecting(true);
        window.location.href = `/church/${tenantSlug}/schedules/${schedule.id}`;
        return;
      }
      ...
      const tableMode =
        ...
        (schedule.schedule_type as ScheduleMode) ??
        "MULTI_SESSION_BY_DATE";

      setScheduleMode(tableMode);

      if (schedule.table_json?.groups) {
        setGroups(schedule.table_json.groups);
      } else {
        setGroups(convertOldTableToGroups(schedule));
      }
    ...
    const loadDivisionPeople = async () => {
      ...
      const mappedMembers =
        membersData?.map((item: any) => ({
          ...
          role: item.role,
          profiles: normalizeJoinedObject<ProfileLite>(item.profiles),
        })) ?? [];

      setDivisionMembers(mappedMembers as DivisionMember[]);

      const { data: servingRolesData, error: servingRolesError } =
        await supabase
      ...
      if (servingRolesError) {
        console.log("Serving roles error:", servingRolesError);
      }
      ...
      if (memberRolesError) {
        console.log("Member serving roles error:", memberRolesError);
      }
      ...
      const mappedMemberServingRoles =
        memberRolesData?.map((item: any) => ({
          ...
          division_serving_roles: normalizeJoinedObject<{
            id: string;
            name: string;
    ...
    return uniqueProfileIds
      .map((itemProfileId) => {
        return divisionMembers.find(
          (member) => member.profile_id === itemProfileId
        )?.profiles;
      })
      .filter((profile): profile is ProfileLite => Boolean(profile));
  };
  ...
  const updateSchedule = async () => {
    if (!scheduleId || !churchId || !divisionId || !title) {
    ...
    if (conflictCount > 0) {
      setStatus(
        `Ada ${conflictCount} cell konflik. Perbaiki dulu sebelum menyimpan.`
      );
      return;
    }
    ...
    try {
      ...
      setStatus(error.message ?? "Terjadi error saat update jadwal.");
      setLoading(false);
    }
  };
          ...
          <div className="mb-10 grid gap-8 xl:grid-cols-[1fr_2fr]">
            <div>
              ...
              <p className="mt-4 max-w-sm text-sm font-medium leading-relaxed text-slate-500">
                Revisi jadwal pelayanan, ubah nama pelayan, geser row/column,
      ...
      <div className="fixed bottom-0 left-0 z-50 w-full border-t border-slate-200 bg-white/85 p-4 shadow-2xl shadow-slate-300/60 backdrop-blur-xl">
        <div className="mx-auto flex max-w-[1800px] items-center justify-between gap-4 px-2 md:px-6">
          <p className="hidden text-sm font-bold text-slate-500 md:block">
            {status ||
              "Pastikan perubahan jadwal sudah benar sebelum menyimpan."}
          </p>
